...
describe Transaction do
  ...
  it"has many products through items" do
    ...
    test_transaction = Transaction.create({:type_trans=>"purchase", :date=>"08/17/2014",
                                            :customer_id=>test_customer.id})
    test_product1 = Product.create({:name=> 'hat', :price=> 40})
    test_product2 = Product.create({:name=> 'shoes', :price=> 75})
    test_item1 = Item.create({:quantity=>1, :product_id=>test_product1.id, :unit=>"ea", :transaction_id=>test_transaction.id})
    test_item2 = Item.create({:quantity=>1, :product_id=>test_product2.id, :unit=>"ea", :transaction_id=>test_transaction.id})
